...
app.get("/justDoIt", function(request, response) {
  console.log(path.join(__dirname, 'learn', 'xyz.html'))
  response.sendFile(path.join(__dirname, 'learn', 'xyz.html'))
})

app.post("/uploadFileWithoutAJAX", (request, response) => {
  
  if(request.files && request.files.profilePic) { // FileExists

    if(request.files.profilePic.truncated) { // FileTruncated (Exceeds Limit)
      response.sendFile(path.join(__dirname, "learn","FileExceedsLimit.html"))
      return;
    } 

    let uploadPath = path.join(__dirname, "uploaded", request.files.profilePic.name);
    console.log(uploadPath);
    let uploadDone = true;
    request.files.profilePic.mv(uploadPath, function() {
      uploadDone = false;
    })
    if(uploadDone) { // FileUploaded
      response.sendFile(path.join(__dirname, "learn","FileUploaded.html"))
    } else { // FileError
      response.sendFile(path.join(__dirname, "learn","FileUploadError.html"))
    }
  } else { // FileMissing
    response.sendFile(path.join(__dirname, "learn","FileMissing.html"))
  }  
})
